...
struct QuakeLocation: Decodable {
    var latitude: Double { self.properties.products.origin.first!.properties.latitude }
    var longitude: Double { self.properties.products.origin.first!.properties.longitude }
    ...
    init(latitude: Double, longitude: Double) {
        self.properties = RootProperties(products: Products(origin: [
            Origin(properties: OriginProperties(
                latitude: latitude,
                longitude: longitude
            ))
        ]))
...
// MARK: - QuakeLocation.OriginProperties + Decodable
